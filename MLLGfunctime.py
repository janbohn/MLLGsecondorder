...
def MLLGfunc(T,rm, N, tau, h,pt,ps, m, rho,alstab, L, tolgmres,matpar,V3,VV,fenics_space,trace_space,trace_matrix):
    print('h is ', h, ' N is', N,' T is', T,' tau is', tau,' pt is', pt)
    ...
    massbd = bempp.api.operators.boundary.sparse.identity(trace_space, bc_space, snc_space)
    C1T= -0.5*np.matmul(assmatrix(massbd.weak_form()),assmatrix(trace_op)) #minus oder plus??
    C0T=1/mu*C1T
    C1= np.transpose(C1T)
    C0=1/mu*C1
    # calderon operator 
    ...
    blocke1[0, 0] = 1.0 / mu * np.sqrt(mu / eps) * caldfac*assmatrix(cald[0, 1])  +tau/2/eps*np.matmul(np.matmul(C0T,Minv),C0)   
    blocke1[0, 1] = -caldfac*cald[0, 0]
    blocke1[1, 0] = caldfac*cald[1, 1]
    blocke1[1, 1] = -mu * np.sqrt(eps / mu) * caldfac*assmatrix(cald[1, 0]) + tau*2*alstab/mu*np.matmul(np.matmul(C1T,Minv),C1)
    
    Lhhs =  assmatrix(BlockedDiscreteOperator(np.array(blocke1)))
    ...
    if pt>1:
        ...
        solu[:]= np.concatenate([np.real(phi_tmp[:]), psiko[:]/2])
    ...
    for j in range(pt-1, int(N)):  #### time stepping: update from timestep t_j to j+1
        ...
        Hj12ko= Hjko+tau/2/mu*Minv.dot(D.dot(Ejko))-tau/2.0*Minv.dot(C1.dot(psiko))- tau*0.5 * Minv.dot(rhsLLGH.weak_form().transpose().dot(vjko))
        ...
        # Right hand side
        Rhhs = -boundary_rhs+np.concatenate([C0T.dot(Ejko/2+Erko/2),C1T.dot(Hj12ko)+2*alstab*tau/mu*C1T.dot(Minv.dot(C1.dot(psiko)))])
        ...
        Hrko= Hj12ko-tau/2/mu*Minv.dot(D.dot(Ejko))-tau/2/mu*Minv.dot(C1.dot(psiko))
