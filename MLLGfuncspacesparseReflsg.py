...
def MLLGfunc(T,rm,onof, N, tau, h,pt,ps, m, rho,alstab, L, tolgmres,matpar,V3,VV,fenics_space,trace_space,trace_matrix,exSolutionCode,dtexSolutionCode):
    ...
    for j in range(pt-1, int(N)):  
        tj12=(j+0.5)*tau
        #### Maxwell update
        Hj12ko,infoo= gmres(mu*Mwv, mu*Mwv.dot(Hjko)+tau/2*D.dot(Ejko)-tau/2.0 *C1.dot(psiko)- onof*tau*mu*0.5 *rhsLLGH.transpose().dot(vjko),tol=tolgmres, x0=Hjko)#,M=MP
        if (infoo > 0):
            print("Failed to converge after " + str(infoo) + " iterations")
        Erko, infoo=gmres((eps+tau*sig/2)*Mwv, (eps-tau*sig/2)*Mwv.dot(Ejko)-tau* D.dot(Hj12ko) +  tau*(tj12)**3* Mwv.dot(JE.vector()[:]),tol=tolgmres,x0=Ejko)   #1.0/3.0*t**3*M0.dot(JccE.vector())+t*(sig*t+2.0)*M0.dot(JE.vector())
        if (infoo > 0):
            print("Failed to converge after " + str(infoo) + " iterations")
        #### Boundary solve
...
-np.real(boundary_rhs[1].projections(wei[0].dual_to_range_spaces[1]))+C1T.dot(Hj12ko)])
        ...
        Hrko, infoo= gmres(mu*Mwv, mu*Mwv.dot(Hj12ko)+tau/2*D.dot(Ejko)-tau/2*C1.dot(psiko),tol=tolgmres,x0=Hjko )#,M=MP
        if (infoo > 0):
            print("Failed to converge after " + str(infoo) + " iterations")
    ...
    if rm>0:
        ...
        class MyExpressionJ(UserExpression):
            def eval(self, value, x):
                s1=sin(np.pi*x[0])
                s2=sin(np.pi*x[1])
                s3=1 #sin(np.pi*x[2]*0.08)
                s1s=s1**4
                s2s=s2**4
                s3s=s3**4
                value[0] = 10*s1s*s2s*s3s
                value[1] = 0
                value[2] = 0 
            def value_shape(self):
                return (3,)       
        JE = MyExpressionJ(degree=ps)
        JE= interpolate(JE, X) #project(Expression(['100.00','000.0','00.0'],degree=ps),X)
        Ej=interpolate(JE, X)
        #Hj=JE
        #mj = mag
        #mj = mj / sqrt(dot(mj, mj))
        #mj = project(mj, V3, solver_type='cg')
        #exSolutionCode = ("0","sqrt(1-1*pow(pow(x[0],3)-0.75*x[0],2))","-1*(pow(x[0],3)-0.75*x[0])")
        exSolutionCode = ("(pow(x[0]-p0,2)+pow(x[1]-p1,2)+pow(x[2]-p2,2))>rr-1e-14 ? 0 : multF*exp(-(tend+0.001)/(tend-t+0.001)/(rr-(pow(x[0]-p0,2)+pow(x[1]-p1,2)+pow(x[2]-p2,2))))*(x[0]-p0)",
            "(pow(x[0]-p0,2)+pow(x[1]-p1,2)+pow(x[2]-p2,2))>rr-1e-14 ? 0 : multF*exp(-(tend+0.001)/(tend-t+0.001)/(rr-(pow(x[0]-p0,2)+pow(x[1]-p1,2)+pow(x[2]-p2,2))))*(x[1]-p1)",
            "(pow(x[0]-p0,2)+pow(x[1]-p1,2)+pow(x[2]-p2,2))>rr-1e-14 ? 1 : sqrt(1-pow(multF,2)*exp(-2*(tend+0.001)/(tend-t+0.001)/(rr-(pow(x[0]-p0,2)+pow(x[1]-p1,2)+pow(x[2]-p2,2))))*(pow(x[0]-p0,2)+pow(x[1]-p1,2)))")
        exSolutionCode = ("(pow(x[0]-p0,2)+pow(x[1]-p1,2)+pow(x[2]-p2,2))>rr-1e-14 ? 0 : (rr-pow(x[0]-p0,2)-pow(x[1]-p1,2)-pow(x[2]-p2,2))*(pow(x[0]-p0,2)+pow(x[1]-p1,2)+pow(x[2]-p2,2))", 
            "(pow(x[0]-p0,2)+pow(x[1]-p1,2)+pow(x[2]-p2,2))>rr-1e-14 ? 0 : (rr-pow(x[0]-p0,2)-pow(x[1]-p1,2)-pow(x[2]-p2,2))*(pow(x[0]-p0,2)+pow(x[1]-p1,2)+pow(x[2]-p2,2))",
            "(pow(x[0]-p0,2)+pow(x[1]-p1,2)+pow(x[2]-p2,2))>rr-1e-14 ? 1 : sqrt(1-2*pow((rr-pow(x[0]-p0,2)-pow(x[1]-p1,2)-pow(x[2]-p2,2))*(pow(x[0]-p0,2)+pow(x[1]-p1,2)+pow(x[2]-p2,2)),2))")
        ...
        exSolutionCode = ("(pow(x[0],2)+pow(x[1],2))>0.25-1e-14 ? 0 : multF*exp(-(tend+0.001)/(tend-t+0.001)/(0.25-(pow(x[0],2)+pow(x[1],2))))*x[0]",
            "(pow(x[0],2)+pow(x[1],2))>0.25-1e-14 ? 0 : multF*exp(-(tend+0.001)/(tend-t+0.001)/(0.25-(pow(x[0],2)+pow(x[1],2))))*x[1]",
            "(pow(x[0],2)+pow(x[1],2))>0.25-1e-14 ? 1 : sqrt(1-pow(multF,2)*exp(-2*(tend+0.001)/(tend-t+0.001)/(0.25-(pow(x[0],2)+pow(x[1],2))))*(pow(x[0],2)+pow(x[1],2)))")
        #mj = interpolate(Expression(exSolutionCode,degree=ps,t=0,tend=0.25,multF=400,rr=0.5,p0=0.5,p1=0.5,p2=0.5), V3) #interpolate(Expression(['1.00', '0.0', '0.0'], degree=ps),V3)#interpolate(Expression(exSolutionCode,degree=ps), V3)#
        class MyExpressionM(UserExpression):
            def eval(self, value, x):
                ...
                s3s=s3**2
                dd=10* s1s*s2s*s3s
                value[0] = dd/np.sqrt(1+dd**2)
                value[1] = 0
                value[2] = 1/np.sqrt(1+dd**2) 
            def value_shape(self):
                return (3,)
        M = MyExpressionM(degree=ps)
        mj= interpolate(M, V3)  
        #mj = mj / sqrt(dot(mj, mj))
        #mj = project(mj, V3, solver_type='cg')
        Hex = project(Expression(['0.00', '0.0', '0.0'], degree=ps), X)
        JccE = project(Expression(['0.00', '0.0', '0.0'], degree=ps), X)
